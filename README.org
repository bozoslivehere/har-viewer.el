...
Pressing =RET= opens a =*HAR Details*= pane showing request and response
headers.  If [[https://github.com/pashky/restclient.el][restclient]] is installed the pane uses ~restclient-mode~ for
...
=C-c C-r= shows the response body and =C-c C-p= shows the request body, each in
a =*HAR Details*= pane set to the appropriate major mode for the content type
(JSON, HTML, CSS, JavaScript). If [[https://github.com/yasuyk/web-beautify][web-beautify]] is installed, and
=har-viewer-beautify-bodies= is set to =t=, the source code for JS/TS, CSS,
HTML, and XML will be automatically beautified in the pane.
